  ...
  def _convert_to_tf_dataset(self, ):
    ...
    for key in self._active_training_keys + self._eval_metric_keys + self._validity_keys:
      if key == 'infeasibility_reason':
        continuetf_actual_dataset[key] = tf.convert_to_tensor(tf_actual_dataset[key], tf.int32)
      tf_actual_dataset[key] = tf.convert_to_tensor(tf_actual_dataset[key], tf.int32)
